...
# KooL Ubuntu-24.04-Hyprland uninstall script #
...
NOTE: Hyprland will be removed automatically once you confirm Uninstall.

WARNING: After uninstallation, your system may become unstable.

Shall we Proceed?" 22 80
...
# Remove files in /usr/local/bin/
for file in "${local[@]}"; do
    if [ -f "/usr/local/bin/$file" ]; then
        sudo rm "/usr/local/bin/$file"
        echo "$file removed from /usr/local/bin."
    else
